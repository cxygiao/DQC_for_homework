...
# 是否显示依赖图
# from Txt.read_txt import read_txt1
...
def count_transfer_queue(gate_list,cut_list,transfer_qubit_list,statistics_gate_labels_list):
    ...
    # networkx.is_directed_acyclic_graph(G)? 是否要加入判断G是否有环？
    for n in G.nodes():
        G.nodes[n]['visited'] = False
    # 变量初始化
    ST = []
...
'''初始解直接搜索 通过队列'''
def direct_calculation_of_tc_queue1(gate_list,cut_list):
    # 全局门列表 ['g0', 'g1', 'g2', 'g4', 'g5', 'g6', 'g8', 'g9']
#    global_gate_num_list = is_global_gate(gate_list, cut_list)[1]
#     print(gate_list)
    # 全局门标签
...
#     print(gate_list)
    ...
    for i in range(len(initial_transfer_queue)-1):
        ...
        if judge_is_same_partation(transfer_qubit_i,transfer_qubit_j,cut_list) == 1 and judge_middle_gates_is_affect_transfer_qubit(middle_gate_list,transfer_qubit_i)==0:
            ...
            # print(gate_j)
            st -= 1
    print('优化后传输代价：' + str(st))
    return st,sto


'''初始解直接搜索 通过队列 分布式方法'''
def direct_calculation_of_tc_queue_distributed(gate_list,cut_list,N):
        ...
        try:
            ...
            for i in range(len(initial_transfer_queue) - 1):
                ...
                if judge_is_same_partation(transfer_qubit_i, transfer_qubit_j,
                                           cut_list) == 1 and judge_middle_gates_is_affect_transfer_qubit(
                        middle_gate_list, transfer_qubit_i) == 0:
                    # print(gate_i)
                    # print(gate_j)
                    st -= 1
            ST.append(st)
...
def test_split_gate_list():
    ...
    cut_list = [14, 14, 14, 14]
    # draw_new_circuit(gate_list)
    ST = []
    for i in range(N):
        print(i, end='')
        try:
            # st = direct_calculation_of_tc(gate_list_list[i], cut_list)
            st = taboo_search2(gate_list_list[i], cut_list)
            ST.append(st)
        except Exception as e:
            print(f"发生错误: {str(e)}")
    print(sum(ST))
...
if __name__ == '__main__':
    ...
    # # # DAG绘制
    # # matrix = draw_matrix_by_gate_list(gate_list)
    # # G = draw_gragh(matrix, gate_list, is_global_gate_list, 1)
    # # for n in G.nodes():
    # #     G.nodes[n]['visited'] = False
    # statistics_gate_labels_list = statistics_gate_labels(gate_list,cut_list)
    # print(statistics_gate_labels_list)
    ...
    # # V = 'g0'
    # # tags = 'q2'
    # # path = dfs(G, V, tags, global_gate_num_list, is_global_gate_list, statistics_gate_labels_list)[0]
    # # print(path)
    #
    # # 量子位传输列表 ['q0', 'q0', 'q2', 'q0', 'q4', 'q1', 'q0', 'q4']
    # initial_transfer_qubit_list = ['q0', 'q0', 'q2', 'q4', 'q4', 'q4', 'q0', 'q4']
    # print(initial_transfer_qubit_list)
    # print(count_transfer_queue(gate_list,cut_list,initial_transfer_qubit_list,statistics_gate_labels_list))
    # transmission_cost = len(count_transfer_queue(gate_list,cut_list,initial_transfer_qubit_list,statistics_gate_labels_list)) * 2
